...
# -*- coding: utf-8 -*-
"""
RedHawk Tool v6.0 - Ultimate Justice Edition (Based on R3D#@X0R_2H1N Logic)
By Farel | Anti-Tamper, Ban System, & Admin Report
Features: Whois, GeoIP, Nmap, Subdomains, Reverse IP, SQL Scan
"""
...
import requests # Library wajib untuk request API
...
class C:
    ...
    PURPLE  = "\033[38;5;129m" # <--- INI YANG KURANG TADI MAS!
    BOLD    = "\033[1m"
...
def show_main_menu(current_key):
    while True:
        ...
        menu_box = (
            f"{C.CYAN}•============================•\n"
            f"{C.GREEN}[1.] Lacak Target (Full Info)\n"
...
def report_issue():
    ...
    mailto_link = f"mailto:{ADMIN_EMAIL}?subject=Laporan REDHAWK v6.0"
...
def start_tracking_interface():
    clear_screen()
    print(f"{C.RED}{C.BOLD}  🕵️  REDHAWK TRACKING MODE 🕵️{C.RESET}\n")
    
    raw_host = input(f"{C.GREEN}  Masukkan Target (contoh: www.google.com) »{C.RESET} ").strip()
    ...
    try:
        ...
        print(f"{C.RED}  ❌ Gagal! Domain '{target}' tidak ditemukan.{C.RESET}")
    except Exception as e:
